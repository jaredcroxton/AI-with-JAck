...
function ratingBgOpacity(value: number | null): string {
  ...
  return intensities[value] || "bg-gray-50";
}
              ...
              {sortedMondays.map((monday) => {
                ...
                return (
                  <td key={dateStr} className="py-2 px-2">
                    <div
                      className={`w-full h-12 rounded-xl flex items-center justify-center text-sm font-semibold transition-all ${ratingBgOpacity(value)} ${ratingTextColor(value)}`}
                    >
                      {value !== null ? value : "-"}
                    </div>
                  </td>
